// The file contents for the current environment will overwrite these during build.
// The build system defaults to the dev environment which uses `environment.ts`, but if you do
// `ng build --env=prod` then `environment.prod.ts` will be used instead.
// The list of which env maps to which file can be found in `.angular-cli.json`.

export const environment = {
  name: 'Development',
  production: false,
  // tslint:disable-next-line:max-line-length
  rss_feeds_default_days: 7,
  GOOGLE_CLIENT_KEY: '31329435502-pjlpea38gv9cjp6599hr51gt2vf1k7t4.apps.googleusercontent.com',
  FACEBOOK_APP_ID: '991696984600301',
  TWITTER_APP_ID: 'Q1ktZThQc3llNTY3dktWNjk4TUE6MTpjaQ',
  LINKEDIN_APP_ID: '775qt148agm2gz',
  BACKEND_GO_URL: 'https://analyticsdev.sravz.com',
  HUGO_URL: 'https://docs.sravz.com/',
  apiUrl: 'https://portfoliodev.sravz.com/api',
  BACKEND_NODE_URL: 'https://portfoliodev.sravz.com',
  BACKEND_PY_TOPIC_NAME: 'production_backend-py',
  BACKEND_RUST_TOPIC_NAME: 'production_backend-rust',
  BACKEND_PY_TOPIC_MAX_CLIENTS: 18,
  DATA_SERVICE_BASE : 'https://portfoliodev.sravz.com',
  PORTFOLIO_SERVICE_BASE : 'https://portfoliodev.sravz.com',
  ANALYTICS_SERVICE_BASE : 'https://analyticsdev.sravz.com',
  QUOTES_SERVICE_BASE : 'https://quotesdev.sravz.com',
  ANALYTICS_SOCKET_SERVICE_BASE : 'wss://analyticsdev.sravz.com',
  CONTABO_BUCKET_NAME: 'sravz-dev',
  testUser: {
    // tslint:disable
    token: {
      expires_in: 3600000,
      access_token: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..xAAbQIOsw3ZXlIxDFnv5NynZy7OfzrvrJYWsy2NEBbA',
    },
    // tslint:enable
    email: 'user@user.user',
  },
  recaptchaSiteKey : '6LcttRATAAAAAI6BLnGfrlswvvgRPEnre9wj2phU',
  JWT_IGNORE_PATHS : ['/auth/login', '/auth/login-guest', '/auth/sign-up', '/auth/request-pass', '/auth/refresh-token', 'contabostorage.com', 's3.amazonaws.com', 'accounts.google.com']
};
